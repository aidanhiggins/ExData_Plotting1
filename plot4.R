...
draw_plot4 <- function(input.data) {
  png("plot4.png")
  par(mfrow=c(2,2))
  with(input.data,
  ...
  with(input.data,
       plot(datetime, Voltage,
            xlab="datetime",
            type="l"))
  with(input.data, plot(datetime, Sub_metering_1, col="black", xlab="",
      ylab="Energy sub metering", type="l"))
  with(input.data, lines(datetime, Sub_metering_2, col="red"))
  with(input.data, lines(datetime, Sub_metering_3, col="blue"))
  legend("topright", col = c('black', 'red', 'blue'), lwd="2", bty="n",
         legend = c('Sub_metering_1', 'Sub_metering_2', 'Sub_metering_3'))
  with(input.data, plot(datetime, Global_reactive_power, type="l"))
  dev.off()
}